...
export const movies: Movie[] = [
  ...
  },
  {
    ...
    year: 1915,
    description:
      "The Stoneman family finds its friendship with the Camerons affected by the Civil War, both fighting in opposite armies. A landmark in film technique despite its controversial content.",
    durationMinutes: 133,
    posterUrl:
...
/**
 * Get playback URL for a movie.
 *
 * In production, this would call your backend to get a signed CloudFront URL.
 * For now, returns a test HLS stream URL.
 *
 * TODO: Replace with actual backend call when ready
 * Example production implementation:
 * ```ts
 * export async function getPlaybackUrl(movieId: string, authToken?: string): Promise<string> {
 *   const response = await fetch(`/api/movies/${movieId}/playback`, {
 *     headers: {
 *       'Authorization': `Bearer ${authToken}`,
 *       // Add any signed URL headers here
 *     },
 *   });
 ...
 * ```
 *
 * @param movieId - The movie slug/ID
 * @returns HLS master playlist URL
 */
export function getPlaybackUrl(movieId: string): string {
  // For testing: Use a public HLS test stream
  // These are commonly available test streams for development

  // Big Buck Bunny - reliable test stream
  const testStreams: Record<string, string> = {
    // Default test stream (Big Buck Bunny)
    default: "https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8",
    // Alternative: Apple's test stream
    apple:
      "https://devstreaming-cdn.apple.com/videos/streaming/examples/img_bipbop_adv_example_ts/master.m3u8",
    // Alternative: Sintel trailer
    sintel: "https://bitdash-a.akamaihd.net/content/sintel/hls/playlist.m3u8",
  };

  // In development, all movies use the default test stream
  // In production, look up the movie's actual hlsMasterUrl
  const movie = getMovieBySlug(movieId);

  if (movie?.hlsMasterUrl) {
    return movie.hlsMasterUrl;
  }

  // Return test stream for development
  return testStreams.default;
}
